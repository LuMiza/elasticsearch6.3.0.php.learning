...
class medoo
{
	...
	{
		if(  preg_match('/^[0-9a-zA-Z_]+$/',trim($this->prefix . $table))===0  ){
	...
	protected function column_push(&$columns)
	{
		...
		//self modify start  
		//�ô���������Ϊ�����������ǿ�м���һ���̶�ֵ���ֶ�  ��ʽ���̶�ֵ as  �ֶ���
		$tempAsCol=array();
		foreach($columns as $keyAsCol=>$valAsCol){
			if(preg_match('/^\s*(\'{1}).*\1\s+as\s+\w+$/i',trim($valAsCol))){
				$tempAsCol[]=trim($valAsCol);
				unset($columns[$keyAsCol]);
			}
		}
		//self modify end
		...
		//����һ��table�в����ڵĹ̶�ֵcolumn
		if( count($tempAsCol)>0 ){
			foreach($tempAsCol as $tempKey=>$tempVal){
				$stack[]=trim($tempVal);
			}
		}
		//����һ��table�в����ڵĹ̶�ֵcolumn
	...
	public function select($table, $join, $columns = null, $where = null,$tempTableFlag=null,$tempTableName=null)
	{
		if(  $columns===true ){
			$tempTableFlag=$columns;
			$tempTableName=$where;
			$columns=null;
			$where=null;
		}
		...
		if($tempTableFlag && is_object($query) && $tempTableName){	
			return " ( ".$query->queryString." ) as ".$tempTableName;
		}
